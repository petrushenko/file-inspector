...
void ht_delete(struct ht_item_t **tbl, char *fname, uint32_t crc32)
{
	...
	if (tmp_ptr && strcmp(tmp_ptr->fname, fname) == 0 && tmp_ptr->crc32 == crc32) {
	...
	while (tmp_ptr && tmp_ptr->pnext) {
		if (strcmp(tmp_ptr->pnext->fname, fname) == 0 && tmp_ptr->pnext->crc32 == crc32) {
...
uint32_t _ht_chngs(ht_item_t **newf, ht_item_t **del, ht_item_t **chn, const char *fname, const char *dir, HANDLE hProgressBar)
{
